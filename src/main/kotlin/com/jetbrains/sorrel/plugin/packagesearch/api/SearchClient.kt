...
import com.jetbrains.sorrel.plugin.packagesearch.api.http.HttpWrapper
...
private val pluginEnvironment by lazy { com.jetbrains.sorrel.plugin.PluginEnvironment() }
...
class SearchClient(
    ...
    private fun packagesInfoByChunk(chunk: List<String>): List<StandardV2Package> {
        ...
        if (chunk.size > maxRequestResultsCount) {
            logWarn(com.jetbrains.sorrel.plugin.SorrelBundle.message("sorrel.search.client.error.too.many.requests.for.range"))
            return emptyList()
        }
        if (chunk.any { it.split(":").size >= maxMavenCoordinatesParts }) {
            logWarn(com.jetbrains.sorrel.plugin.SorrelBundle.message("sorrel.search.client.error.no.versions.for.range"))
            return emptyList()
        }
